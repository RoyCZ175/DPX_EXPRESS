...
/* ── VISTA 2: subcategorías (ej. Eventos / Personalizados) ── */
function mostrarSubcategorias(parentCat) {
  ...
  setTimeout(() => {
    ...
    const subs = SUBCATEGORIAS[parentCat];
    grid.innerHTML = subs.map((key, i) => {
      const cat   = CATEGORIAS[key];
      const count = todosLosProductos.filter(p => p.categoria === key).length;
      const bgStyle = cat.img
        ? `style="animation-delay:${i * 0.1}s; background-image: url('${cat.img}');"`
        : `style="animation-delay:${i * 0.1}s;"`;
      return `
        <div class="cat-card cat-pasteleria" data-cat="${key}" ${bgStyle}>
          <div class="cat-card-overlay"></div>
          <div class="cat-card-icon">${cat.icon}</div>
          <h3>${cat.label}</h3>
          <p>${cat.desc}</p>
